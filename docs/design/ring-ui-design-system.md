...
- **Any exact-slice construction** — since 2026-09-03 NemoLoop renders Dory-style fan blades (`BladeLayout` in `RingGeometry.swift` is the single layout source: every blade a fixed 30° sector, edge-to-edge with its neighbours — up to 11 blades tiling 330° — in a fan centered on up, wrap gap centered on down, its size `360° − 30°·N`; pointer in the gap selects nothing, and hit boundaries fall on blade edges so hover matches the visible seams). Each item is a self-contained view (logo centered inside the blade view) tilted with a single `rotation3DEffect` about its canvas-space tangential axis `(cos θ, sin θ, 0)` — near-big-far-small; do NOT compose `rotation3DEffect` with `rotationEffect` (breaks placement). Ascending z-order; per-blade seam shadow; no continuous ring backing. **Platform materials are banned inside the blade view** (VisualEffectView, `.ultraThinMaterial`, macOS 26 `glassEffect`): they composite above their SwiftUI siblings and swallow them, and NSViews ignore 3D transforms — use plain translucent fills. See `docs/superpowers/specs/2026-09-03-fan-blade-ring-design.md`.
