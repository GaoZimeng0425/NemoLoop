...
- **Any exact-slice construction** — since 2026-09-03 NemoLoop renders Dory-style overlapping fan blades (`WedgeShape.bladeWidth` > slot pitch; each blade independently tilted via `rotationEffect(slot + tilt)` about an arc center offset tangentially by `bladePivotOffset`; ascending z-order so the last blade overlaps the first at the wrap; per-blade seam shadow; upright icons above all blades; no continuous ring backing). Per-blade macOS 26 `glassEffect` inside a compositing group renders only the last shape and suppresses siblings — each blade uses its own `VisualEffectView` material; do not reintroduce glassEffect here. See `docs/superpowers/specs/2026-09-03-fan-blade-ring-design.md`.
