...
3. **收口缺口（v3，v5 泛化）**：扇面组**以正上方为中心对称**排布，缺口居中正下方；`N ≤ 11` 时缺口 = `360° − 30°·N ≥ 30°`，`N > 11` 时间距压缩到 `(360°−30°−30°)/(N−1)`（此时才出现叠压）以保证缺口不小于 30°。
...
- 槽位中心 `centerAngle(i) = start + bladeWidth/2 + i·pitch`，`start = −span/2`（**扇面组以正上方为中心对称**，缺口居中正下方；`span = (N−1)·pitch + bladeWidth`）。
...
- N=1：单片居中正上；N=6（当前 `wedgeCount`）：覆盖 −90°…+90°，正下方 180° 缺口。
