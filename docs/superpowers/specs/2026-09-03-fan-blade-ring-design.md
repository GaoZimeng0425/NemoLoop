...
### v6.2：hover 原地高亮（不顶 z 序）+ 白色提亮 + 发牌提速

三项用户指令改动（2026-09-04，渲染探针双方案验证）：

1. **hover 不再浮到最上层**：`zIndex` 恒为 `bladeCount−i`（去掉 `isHot` 时 `+bladeCount`），热片只原地外滑 6pt——被逆时针邻居压住边缘、仍压住顺时针邻居，叠压语言不再被 hover 打断。
2. **hover 高亮 = 白色提亮，弃 accent 渐变**。根因：`accentGradient` 是 `accentColor → accentColor 混蓝 45%`，而用户系统强调色为**橙**（`AppleAccentColor=1`），橙+蓝=棕褐泥色糊在近白卡纸上。且近白卡纸上白色叠层**必须近乎不透明才可见**（白 40% 只移动亮度约 2%）——定案按配色方案分档：浅色 `highlightFill = 白 95%`、深色 `白 32%`（炭色上白色读得强，32% 已是"点亮的深色卡"）；空槽高亮浅色 0.16→0.45、深色维持 0.18。accent 三 token 移除，`RingPaletteTests` 加字面值守卫。
3. **出现（deal-out）动画提速约 20%**：`spring response 0.30→0.24`、每片 stagger `32ms→26ms`。

验证：探针真实窗口渲染浅/深两方案截图——浅色白提亮一眼可辨且不破磨砂语言、深色以像素直方图证实（卡面 ≈33、高亮卡面 ≈65-100，视觉模型对极暗图会误判"全黑"，勿信）；30 项单测全过；待用户 ⌃T 实机验收。
